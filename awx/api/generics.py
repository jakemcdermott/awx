...
class CopyAPIView(GenericAPIView):
    ...
    def _decrypt_model_field_if_needed(obj, field_name, field_val):
        if isinstance(field_val, dict):
            for sub_field in field_val:
                if isinstance(sub_field, (str, unicode)) \
                        and isinstance(field_val[sub_field], (str, unicode)):
                    try:
        ...
        elif isinstance(field_val, (str, unicode)):
    ...
    def copy_model_obj(old_parent, new_parent, model, obj, creater, copy_name='', create_kwargs=None):
        ...
        m2m_to_preserve = {}
        o2m_to_preserve = {}
        is_create_frozen = bool(create_kwargs)
        ...
        for field in model._meta.get_fields():
            ...
            if field.one_to_many:
                ...
                if not field_val:
                    create_kwargs.pop(field.name, None)
                    continue
                if field_val == old_parent:
                    create_kwargs[field.name] = new_parent
                elif is_create_frozen and field.name not in fields_to_preserve:
                    create_kwargs.pop(field.name, None)
                elif not is_create_frozen and field.name in fields_to_preserve:
                ...
                if not old_parent:
                    ...
                    create_kwargs[field.name] = field_val
            elif field.name in fields_to_preserve or not is_create_frozen:
